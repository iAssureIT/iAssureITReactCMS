...
exports.getPersonList = (req,res,next)=>{ 
    ...
    PersonMaster.find({company_Id: req.body.company_Id,type:req.body.type,status:{$in:req.body.status}},{ firstName: 1, middleName:1, lastName: 1,contactNo:1,profilePhoto:1,email:1,type:1,status:1,statusLog:1})
    .sort({createdAt : -1})
    .then(data=>{
        res.status(200).json( data );
    })
        ...
        .then(validDrivers=>{
            ...
            if(validDrivers && validDrivers.length > 0){
                for(var i=0; i<validDrivers.length; i++){
                    for(var j=0; j<validDrivers[i].Documentarray.length; j++){
                        if(validDrivers[i].Documentarray[j].documentName === "License" && new Date(validDrivers[i].Documentarray[j].documentValidTo) <= new Date(desiredToDate)) {
