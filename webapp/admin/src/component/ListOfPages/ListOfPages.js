...
getListOfPages(){
	/*/get/list*/
	axios
			.get('http://qaiassureitapi.iassureit.com/api/pages/get/list')
			.then((response)=>{        
			      	this.setState({
	...
	// console.log("id delet", URL);
	 swal({
        ...
        .then((success) => {
            if (success) {
            	axios
			    .delete("http://qaiassureitapi.iassureit.com/api/pages/delete/"+URL)
			    .then((response)=>{
			     	this.getListOfPages();
...
editPage(event){
	...
	var URL= event.target.id;
    axios
      .get("http://qaiassureitapi.iassureit.com/api/pages/get/"+URL)
      .then((response)=>{
...
updatePageData(){
 ...
 const formValues = {
   ...
   console.log("t=-=-=-=-",this.state.UrlId);
    ...
    axios
          .patch('http://qaiassureitapi.iassureit.com/api/pages/patch/'+this.state.UrlId,formValues)
          .then((res)=>{
...
submitData(){
	if($('#newTemplateForm').valid()){
		var AttachmentValues = {    
   		...
   		axios
			.post('http://qaiassureitapi.iassureit.com/api/pages/post',AttachmentValues)
		  	.then((response)=>{
		    // handle success
			...
			<div className="box-body1">
				<div className="col-lg-12 col-md-12 col-xs-12 col-sm-12 box-body2">
					...
					<div className="col-lg-12 col-md-12 col-xs-12 col-sm-12">
								        ...
								        <a className="pull-right CNPBtn" data-toggle="modal" data-target="#myModal">
								        	<button type="button" class="btn btn-primary">Create New Page </button>
								        </a>
										...
										this.state.ListOfPages.map((result, index)=>{
											...
											return(
											    <tr key={index}>
											      ...
											      <td><i className="fa fa-edit deletIcon" id={result.pageURL} data-toggle="collapse" data-toggle="modal" data-target="#myModal" onClick={this.editPage.bind(this)} data-placement="top" title="Edit Page"></i>&nbsp;&nbsp;&nbsp;&nbsp;
